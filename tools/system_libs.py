...
def calculate(temp_files, in_temp, stdout_, stderr_, forced=[]):
  ...
  def build_libc(lib_filename, files, lib_opts):
    ...
    for src in files:
      ...
      commands.append([shared.PYTHON, shared.EMCC, shared.path_from_root('system', 'lib', src), '-o', o] + musl_internal_includes + trace_opts + lib_opts)
      o_s.append(o)
  ...
  def build_libcxx(src_dirname, lib_filename, files, lib_opts):
    ...
    for src in files:
      ...
      commands.append([shared.PYTHON, shared.EMXX, srcfile, '-o', o, '-std=c++11'] + trace_opts + lib_opts)
      o_s.append(o)
    run_commands(commands)
    shared.Building.link(o_s, in_temp(lib_filename))
    return in_temp(lib_filename)
